...
localBi.filterTable = function(tableDefinition) {
	...
	var filterFields = [];
	for (var fieldIndex = 0; fieldIndex < tableDefinition.tableFields.length; fieldIndex++) {  //build filterFields
		var tableField = tableDefinition.tableFields[fieldIndex];  //this will be an array of field values
		var filterValues = [];
		for (var valueIndex = 0; valueIndex < tableField.length; valueIndex++) {  //value data starts at index 1, but we include the header (which is the field name) so we don't lose any record index references
		 ...
		 */
		var filterValue = {  //@typedef filterValue
	...
	var filterRecords = [];
	for (var recordIndex = 1; recordIndex < tableDefinition.tableRecords.length; recordIndex++) {  //build filterRecords - record data starts at index 1, skip header
		var recordValues = [];
		for (var fieldIndex = 0; fieldIndex < filterFields.length; fieldIndex++) {  //we should have the same number of fields in records
			var filterField = filterFields[fieldIndex];  //get field from fieldIndex
			var valueIndex = tableDefinition.tableRecords[recordIndex][fieldIndex];  //tableRecords contains index of filterValue for the same fieldIndex
			var filterValue = filterField.fieldValues[valueIndex];
		 ...
		 */
		var filterRecord = {  //@typedef filterRecord
	 ...
	 */
	 var filterTable = {  //@typedef filterTable
		...
		findField(fieldName) {  //find filterField if it exists, else return null
			var fieldIsFound = false;  //to check if filterField exists
			var fieldIndex = 0;  //position of found filterField
		...
		filter(filterSpecifications) {  //apply filters, specification uses fieldName
			 ...
			 */
			for (var filterIndex = 0; filterIndex < filterSpecifications.length; filterIndex++) {  //set all filterValues' filter state as defined by filterSpecifications
				var filterSpecification = filterSpecifications[filterIndex];
				var fieldsToFilter = [];  //placeholder for subset of fields to filter
				...
				for (var fieldIndex = 0; fieldIndex < fieldsToFilter.length; fieldIndex++) {  //potentially we can set all filterFields' states
			...
			for (var fieldIndex = 0; fieldIndex < filterTable.filterFields.length; fieldIndex++) {  //reset all filterValue states for all filterFields
				var filterField = filterTable.filterFields[fieldIndex];
				for (var valueIndex = 0; valueIndex < filterField.fieldValues.length; valueIndex++) {
					var filterValue = filterField.fieldValues[valueIndex];
			...
			for (var recordIndex = 0; recordIndex < filterTable.filterRecords.length; recordIndex++) {  //scan all filterRecords to calculate active filterRecords and update active filterValues
				var filterRecord = filterTable.filterRecords[recordIndex];
				...
				for (var fieldIndex = 0; fieldIndex < filterRecord.recordValues.length; fieldIndex++) {  //set the state of each field's filterValue that exists in the record
					var filterValue = filterRecord.recordValues[fieldIndex];
		...
		aggregate(aggregateSpecification) {  //build hierarchy of dimensions and calculate measures.
			...
			function buildGroups(dimensionSpecifications) {  //group active filterRecords according to dimensionSpecifications and returns the groups of filterRecords
				...
				for (var dimensionIndex = 0; dimensionIndex < dimensionSpecifications.length; dimensionIndex++) {  //build indexed dimensionSpecifications - faster to do this upfront
				...
				var filterGroups = [];  //result set of filterGroups
				for (var recordIndex = 0; recordIndex < filterTable.filterRecords.length; recordIndex++) {  //determine group by field combos - scan through all rows, but only use the active ones for groups
					var filterRecord = filterTable.filterRecords[recordIndex];
					if (filterRecord.recordIsActive == true) {  //only do active rows
						 ...
						 */
						var filterGroup = {  //@typedef hierarchyGroup
						...
						for (var dimensionIndex = 0; dimensionIndex < dimensionSpecifications.length; dimensionIndex++) {  //build filterGroup's groupValues to have columns as per dimensionSpecifications
						...
						var groupIsFound = false;  //to check if filterGroup exists
						var groupIndex = 0;  //position of found group in filterGroups
						while (groupIsFound == false && groupIndex < filterGroups.length) {  //search until we can confirm filterGroup does exist in the existing filterGroups
							var existingGroup = filterGroups[groupIndex];
							var groupIsSame = true;  //remains true until we find one value that doesn't match, exit early
							var fieldIndex = 0;  //to compare the same dimension indices across groups
			...
			function buildHierarchy(filterGroup, hierarchyChildren, hierarchyLevel) {  //recursively build the hierarchy of dimensions, one level per dimension, for one filterGroup
				var nodeValue = filterGroup.groupValues[hierarchyLevel].valueName;  //build list of unique values, for each field in the group
				var valueIsFound = false;  //to check if value exists
				var valueIndex = 0;  //position of found value
				...
				var hierarchyNode = hierarchyChildren[valueIndex];
			...
			function aggregateHierarchy(measureSpecifications, nodeHierarchy) {  //aggregate the active filterRecords for each level in the hierarchy. Passes filterTable and node through to aggregation function defined in tableFunctions.
				...
				for (var nodeIndex = 0; nodeIndex < nodeHierarchy.length; nodeIndex++) {
					var node = nodeHierarchy[nodeIndex];
					if (node.children.length > 0) aggregateHierarchy(measureSpecifications, node.children);  //dig deeper
					var measures = [];
					for (var measureIndex = 0; measureIndex < measureSpecifications.length; measureIndex++) {  //measures are as per specification
			...
			var filterGroups = buildGroups(aggregateSpecification.dimensions);  //group by the dimensionSpecifications and return a group of records for each unique combination of dimension values
			var filterHierarchy = [];  //each successive dimension will form another level in the hierarchy
			for (var groupIndex = 0; groupIndex < filterGroups.length; groupIndex++) {
